...
const Val= require('../models/synval')
var stuid=0
var ename=0
var count=0
var mm=0
...
router.post('/adddetails',async (req,res)=>{
	...
	if(req.body.formtype=="abstract"){
		res.render("abstract.ejs",{stuid:stuid, ename:ename, dates:dates, formtype:formtype})
	}
	else if(req.body.formtype=="synopsis")
		{
			res.render("synopsis.ejs",{stuid:stuid, ename:ename, dates:dates, formtype:formtype})
		}
})
router.post('/success/:stuid/:ename/:dates/:formtype/',async (req,res)=>{
	...
	var a=req.param.stuid
	var b=req.param.ename
	var c=req.param.dates
	var d= req.params.formtype
	...
	const user= new User({
		Evaluator:req.params.ename,
		Studentid: req.params.stuid,
		Date: req.params.dates,
...
router.post("/final/:stuid/:ename/:dates/:formtype/",async (req,res)=>{
	console.log(req.param.ename)
	...
	if(req.body.a5==1)
	...
	else if(req.body.a5==2)
		{
			v30=4
			co+=v30
			vv30="Proposed methodology is partially satisfactory"
		}
	else if(req.body.a5==3)
	{
		v30=8
		mm+=v30
		vv30="Proposed methodology is satisfactory-8"
	}
	...
	else if(req.body.c8==3)
	{
		vas3=2
		mm+=vas3
		vvs3="Realistic Progress Charted"
	}
	var first= count/17
	var second= co/58
    var third= mm/100
...
router.get('/getval', async (req,res)=>{
	const getvalue= await Fast.find()
	var arr = [];
	var brr= [],i,j;
for (var id in getvalue) {		
    arr.push(getvalue[id]["Studentid"]);
	
}
	var len= arr.length
	res.render("resultsyn.ejs",{arr:arr, len:len})
})
router.post("/value",async (req,res)=>
		  {
	console.log(req)
	res.render("fin.ejs",{sel:req.body.selectpicker})
	
})

router.get("/getdat", async (req,res)=>{
	try{
		const gotuser= await Fast.find()
		res.json(gotuser)
	}
	catch(err){
